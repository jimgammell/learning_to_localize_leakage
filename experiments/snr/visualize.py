...
def visualize_snr_for_ascadv2(
    ...
    assert dataset_id in ['ascadv2']
    from leakage_localization.datasets.ascadv2 import repr_target, TARGET_BYTE, TARGET_VARIABLE
    for file in dest.iterdir():
        if not file.name.endswith('.npy'):
            continue
        target_var, partition, _ = file.name.split('.')
...
def visualize_snr_for_dataset(
    ...
    elif dataset_id in ['ascadv2']:
        visualize_snr_for_ascadv2(dataset_id=dataset_id, partition_id=partition_id, dest=dest, overwrite=overwrite)
