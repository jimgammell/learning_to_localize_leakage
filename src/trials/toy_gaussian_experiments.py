    ...
    def run_multi_xor_experiments(self, logging_dir, max_leaky_pair_count: int = 12, run_baselines: bool = True):
        ...
        for seed in range(self.seed_count):
            for leaky_pair_count in range(max_leaky_pair_count+1):
                ...
                if not os.path.exists(os.path.join(trial_dir, 'leakage_assessment.npy')):
                    set_seed(seed)
                    profiling_dataset = SimpleGaussianDataset(random_count=1, second_order_pair_count=2**leaky_pair_count)
                    attack_dataset = SimpleGaussianDataset(random_count=1, second_order_pair_count=2**leaky_pair_count)
                    all_trainer = ALLTrainer(
                        profiling_dataset, attack_dataset, 
                        default_data_module_kwargs={'train_batch_size': int(0.08*len(profiling_dataset)), 'data_mean': np.array([0.0]), 'data_var': np.array([1.0]), 'num_workers': 0},
                        default_training_module_kwargs={**self.leakage_localization_kwargs}
